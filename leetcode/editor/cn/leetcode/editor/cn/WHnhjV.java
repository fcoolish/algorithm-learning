...
//
//每位勇者初始都拥有一些能量宝石， `gem[i]` 表示第 `i` 位勇者的宝石数量。现在这些勇者们进行了一系列的赠送，`operations[j] = [
//x, y]` 表示在第 `j` 次的赠送中 第 `x` 位勇者将自己一半的宝石（需向下取整）赠送给第 `y` 位勇者。
//
...
//- `0 <= operations[i][0], operations[i][1] < gem.length` Related Topics 数组 模拟 
//👍 2 👎 0
...
public class WHnhjV{
    public static void main(String[] args){
        ...
        int[] gem = new int[]{0,2,5,4};
        int[][] op = new int[][]{{3,2},{3,2},{1,3},{0,2},{3,0},{3,1},{0,3},{2,1},{3,0}};
        solution.giveGem(gem,op);
    }
    //leetcode submit region begin(Prohibit modification and deletion)
class Solution {
    public int giveGem(int[] gem, int[][] operations) {
        for(int[] op:operations){
            gem[op[1]] = gem[op[1]] + gem[op[0]]/2;
