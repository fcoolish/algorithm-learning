...
//
//在新手村中，各位勇者需要采集一些果实来制作药剂。`time[i]` 表示勇者每次采集 `1～limit` 颗第 `i` 种类型的果实需要的时间（即每次最多可
//以采集 `limit` 颗果实）。
//
//当前勇者需要完成「采集若干批果实」的任务， `fruits[j] = [type, num]` 表示第 `j` 批需要采集 `num` 颗 `type` 类
//型的果实。采集规则如下：
...
//- `1 <= fruits[i][1] < 10^3`
//- `1 <= limit <= 100` Related Topics 数组 👍 1 👎 0

package leetcode.editor.cn;
public class PTXy4P{
    public static void main(String[] args){
        Solution solution = new PTXy4P().new Solution();
    }
...
class Solution {
    public int getMinimumTime(int[] time, int[][] fruits, int limit) {
        ...
        for(int i =0;i < fruits.length;i++){
            int count = fruits[i][1] % limit == 0 ? fruits[i][1] / limit : (fruits[i][1] / limit + 1);
            res += time[fruits[i][0]] * count;
        }
        return res;
    }
}
