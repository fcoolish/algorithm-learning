...
// s 仅由数字（0 - 9）组成。 
// 
// Related Topics 字符串 模拟 👍 13 👎 0

package leetcode.editor.cn;

public class CalculateDigitSumOfAString{
    public static void main(String[] args){
        Solution solution = new CalculateDigitSumOfAString().new Solution();
    ...
    public String digitSum(String s, int k) {
        ...
        while (ans.length() > k){
           ...
           for(int i = 0;i < ans.length();i +=k){
               String str = ans.substring(i,Math.min(l,i + k));
               int num = 0;
               for(int j =0;j < str.length();j++){
                   num  += Integer.valueOf(str.charAt(j) - '0');
               }
                sb.append(num);
            }
           ans = sb.toString();
