...
//
// Related Topics 树 深度优先搜索 回溯 二叉树 👍 422 👎 0
...
class Solution {
    ...
    public void dfs(TreeNode root,List<Integer> list,int tar){
        if(root == null)return;
        tar -= root.val;
        list.add(root.val);
        ...
        dfs(root.left,new ArrayList<>(list),tar);
        dfs(root.right,new ArrayList<>(list),tar);
    }
}
